...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    let output = createMenu(); // Retorno: { fetchMenu: () => {}, ... }
    assert.ok(Object.keys(output).includes('fetchMenu'));
    // ```
    // TESTE 2: Verifique que, dado que a função createMenu foi chamada com o objeto: `{ food: {}, drink: {} }`, 
    // verifique que 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são somente `food` e `drink`.
    // ```
    let input = { food: {}, drink: {} }
    output = createMenu(input);
    let fetch = output.fetchMenu(); // Retorno: { food: {}, drink: {}};
    assert.ok(Object.keys(input).includes('food', 'drink'));
    // ```
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    input = { food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} };
    output = createMenu(input);
    fetch = output.fetchMenu();
    assert.deepStrictEqual(fetch, input);
    // ```
