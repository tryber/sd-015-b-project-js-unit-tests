...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const chavesObjetoRetornado = Object.keys(objetoRetornado);
    assert.strictEqual(chavesObjetoRetornado.includes('fetchMenu'), true)
    ...
    // ```
    objetoRetornado = createMenu({ food: {hamburguer: 1}, drink: {água: 50}, });
    
    assert.deepStrictEqual(objetoRetornado.fetchMenu(), { food: {hamburguer: 1}, drink: {água: 50}, });
    ...
    // ```
    objetoRetornado =  createMenu({ food: {'coxinha': 10, 'sopa': 1}, drink: {'agua': 5, 'cerveja': 2} })
    objetoRetornado.order('coxinha');
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    assert(objetoRetornado.pay(), 27.50);
