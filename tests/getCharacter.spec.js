...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    assert.notStrictEqual(getCharacter('ARYA'), undefined);
    assert.notStrictEqual(getCharacter('BRIeNNE'), undefined);
    assert.notStrictEqual(getCharacter('melissandrE'), undefined);
    // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    assert.strictEqual(getCharacter('anythingElse'), undefined);
  });
});
