...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    assert.deepStrictEqual(getCharacter('Arya'), objetoArya);
    ...
    assert.deepStrictEqual(getCharacter('Brienne'), objetoBrienne);
    // Teste se a função retorna o objeto correto para o parâmetro 'Melissandre',
    const objetoMelissandre = {
      name: 'Melissandre',
      class: 'Necromancer',
      phrases: ['Death By Fire Is The Purest Death.', 'For The Night Is Dark And Full Of Terrors.'],
    }
    assert.deepStrictEqual(getCharacter('Melissandre'), objetoMelissandre);
    // Teste se a função se os parâmetros não são Case Sensitive.
    assert.deepStrictEqual(getCharacter('arya'), objetoArya);
    assert.deepStrictEqual(getCharacter('brienne'), objetoBrienne);
    assert.deepStrictEqual(getCharacter('brienne'), objetoBrienne);
    // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    assert.strictEqual(getCharacter('Hodor'), undefined);
  });
});
