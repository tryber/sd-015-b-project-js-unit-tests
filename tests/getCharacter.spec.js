...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const error = 'Opa!, código errado, verifique-o!';
    assert.strictEqual(getCharacter(), undefined, error);
    const arya = { name: 'Arya Stark', class: 'Rogue', phrases: ['Not today', 'A girl has no name.'] };
    assert.deepStrictEqual(getCharacter('Arya'), arya, error);
    const brienne = { name: 'Brienne Tarth', class: 'Knight', phrases: ['Im No Lady, Your Grace.', 'I, Brienne Of Tarth, Sentence You To Die.'] };
    assert.deepStrictEqual(getCharacter('Brienne'), brienne, error);
    ...
    assert.deepStrictEqual(getCharacter('Melissandre'), melissandre, error);
    assert.deepStrictEqual(getCharacter('Arya'), getCharacter('ARYA'), error);
    assert.strictEqual(getCharacter('Erivam'), undefined, error);
  });
});
