...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    const firtProduct = productDetails('Alcool gel', 'Máscara')[0].details.productId;
    const secondProduct = productDetails('Alcool gel', 'Máscara')[1].details.productId;
    assert.deepStrictEqual(firtProduct.indexOf('123') === firtProduct.length - 3, true);
    assert.deepStrictEqual(secondProduct.indexOf('123') === secondProduct.length - 3, true);
  });
});
