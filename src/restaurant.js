...
const payMenu = function() {
  const pedidos = this.fetchMenu();
  let total = 0;

  for(let index = 0; index < 5; index += 1) {
    pedido = this.consumption[index];
    console.log(Object.keys(pedidos.food))
    if(Object.keys(pedidos.food).includes(pedido)) {
      total += pedidos.food
    }
  }
};
...
console.log(createMenu().pay());
// Parâ

// - Uma chave `pay` que, quando chamada, invoca uma função que so       ma o valor de todos os pedidos e dá o preço com acréscimo de 10%.
