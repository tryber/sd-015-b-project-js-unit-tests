...
const somaDeTudoDrink = () => {
  const consumido = restaurant.consumption;
  let soma = 0;
  for (let index = 0; index < consumido.length; index += 1) {
    if (consumido[index] === 'agua') {
      soma += 3.9;
    }
    if (consumido[index] === 'cerveja') {
      soma += 6.9;
    }
  }
  return soma;
...
const somaDeTudo = () => {
  let soma = somaDeTudoFood();
  soma += somaDeTudoDrink();
  let result = parseFloat((soma * 1.1).toPrecision(6));
  return result;
};
