...
const createMenu = (menu) => {
  const objeto = {
    fetchMenu: () => menu,
    consumption: [],
    order: (foods) => objeto.consumption.push(foods),
    pay: () => {
      let sum = 0;
      objeto.consumption.forEach((element) => {
        const comida = menu.food[element];
        if (comida !== undefined) sum += comida;
        const drinks = menu.drinks[element];
        if (drinks !== undefined) sum += drinks;
      });
