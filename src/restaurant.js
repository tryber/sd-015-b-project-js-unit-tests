...
const createMenu = (menu) => {
  const returnedObject = {
    fetchMenu: () => menu,
    consumption: [],
    order: (order) => returnedObject.consumption.push(order),
    pay: () => {
      let total = 0;      
      returnedObject.consumption.forEach((order) => {
        const foods = menu.food[order];
        const drinks = menu.drink[order];
        if (foods !== undefined) total += foods;
        if (drinks !== undefined) total += drinks;
      });
